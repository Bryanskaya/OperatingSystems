...
static ssize_t fortune_write(struct file *filp, const char __user *buff, size_t max_bytes_to_write, loff_t *f_pos)
{
    int free_space = (MAX_COOKIE_LENGTH - cookie_index) + 1;

    if (free_space < max_bytes_to_write)
    {
        printk(KERN_ERR "my_fortune: cookie_pot is overflowed");
        return -EFAULT;
    }

    if (copy_from_user(&cookie_pot[cookie_index], buff, max_bytes_to_write))
    {
...
int simple_proc_open(struct inode *ind, struct file *filp)
{
